...
#3D wireframe
gp.run('#set terminal postscript eps color;set output "Maungawhau1.eps"
set hidden3d
splot "volcano.txt" matrix w l', TRUE )

#3D color surface
gp.run('#set terminal postscript eps color;set output "Maungawhau2.eps"
set pm3d
splot "volcano.txt" matrix w pm3d', TRUE )

#2D heatmap
gp.run('#set terminal postscript eps color;set output "Maungawhau3.eps"
set pm3d map
splot "volcano.txt" matrix w pm3d', TRUE )

#2D countour plot
gp.run('#set terminal png;set output "Maungawhau4.png"
#unset clabel;
unset surface
set view map
set contour base
...
gp.run('#set terminal png;set output "Maungawhau5.png"
set pm3d at b
splot "volcano.txt" matrix w l', TRUE )

#3D color surface + 2D contour
gp.run('#set terminal png;set output "Maungawhau6.png"
set contour base
set hidden3d
splot "volcano.txt" matrix w pm3d nocontour, "" matrix  w lines nosurface', TRUE )

#3D surface + contour
gp.run('#set terminal postscript eps color;set output "Maungawhau7.eps"
set contour surface
set hidden3d
...
tmpfile1<-tempfile()
tmpfile2<-tempfile()
gp.run('#set terminal postscript eps color;set output "Maungawhau8.eps"
...
gp.image.plot("MaungawhauZOOM16.png")

#use GIMP to crop and rescale
#crop 303x434 
gp.image.crop("MaungawhauZOOM16.png","MaungawhauZOOM16crop.png",150, 92,453,526)

#rotate 180 degrees 
gp.image.rotate("MaungawhauZOOM16crop.png","MaungawhauZOOM16cropRot.png",180)

#resize to 61x87
gp.image.resize("MaungawhauZOOM16cropRot.png","Maungawhau61x87.png",61,87)

gp.image.plot("Maungawhau61x87.png")
...
#3D DEM with points
gp.run('#set terminal png;set output "Maungawhau10.png"
set view equal_axes xy
splot "Maungawhau2.dat" u 1:2:3:4 w p pt 5 ps 1 lc rgb var notitle', TRUE )

#3D DEM with palette
gp.run('#set terminal png;set output "Maungawhau11.png"
...
PNGdataIndexed<-gp.CreateIndexFromMatrixAndPalette(PNGdata, paletteRGB) # create an indexed color matrix
p<-matrix(c(t(PNGdataIndexed)),ncol=1)
gp.matrix2XYdata('volcano.txt',testmap %s% 'XYndx.dat',p,surfacegrid=FALSE,TRUE)
NpaletteColors<-length(paletteRGB)-1 # number of palette colors starting from zero

gp.run('#set terminal png;set output "Maungawhau12.png"
...
## download a Google Maps tile and use its coordinates to clip a DEM (in geoTIFF format) 
# New Zealand National Digital Elevation Model (North Island) from Landcare Research, Sourced from LINZ Geodetic Database. Crown Copyright Reserved.
# from http://lris.scinfo.org.nz/layer/131-nzdem-north-island-25-metre/
# save data from coords: 174.7581, 174.7718, -36.88127, -36.87013
# the file saved as 1-0001-0001.tif
...
gp.run('splot "MaungawresXY.dat" u 1:2:3 w l',TRUE)

# Rotate and resize the satellite image.
gp.image.rotate("MaungawhauZOOM16.png","MaungawhauZOOM16rotated.png",270)
gp.image.resize("MaungawhauZOOM16rotated.png","Maungawhau220x270.png",44*5,54*5)
gp.image.plot('Maungawhau220x270.png')

pngmap<-'Maungawhau220x270'
PNGdata<-gp.PNG2color(pngmap %s% '.png') # get the color matrix from the PNG file
paletteRGB<-gp.CreatePaletteFromMatrix(PNGdata) # create a palette
gp.RGB1to3channels(paletteRGB,fileRGB3channel=pngmap %s% '.pal') # save the palette to a file with separated RGB components
PNGdataIndexed<-gp.CreateIndexFromMatrixAndPalette(PNGdata, paletteRGB) # create an indexed color matrix
NpaletteColors<-length(paletteRGB)-1 # number of palette colors starting from zero

# create a gnuplot data file with color and DEM
gp.xydata2matrix('MaungawresXY.dat','Maungaw640.dat')
gp.run('set pm3d;splot "Maungaw640.dat" matrix w pm3d',TRUE)
p<-matrix(c(t(PNGdataIndexed)),ncol=1)
gp.matrix2XYdata('Maungaw640.dat', 'Maungawhau3.dat',p,surfacegrid=FALSE,TRUE)

gp.run('#set terminal pngcairo;set output "Maungawhau13.png"
...
unset colorbox
set hidden3d
splot "Maungawhau3.dat" u 1:2:3:4 w pm3d notitle', TRUE )
